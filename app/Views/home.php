    ...
    <a href="<?= $baseUrl ?>/articles/<?= htmlspecialchars($article['slug']) ?>" class="group block space-y-3">
        ...
        <div class="relative w-full h-56 bg-gray-200 rounded-lg overflow-hidden">
            ...
            <img src="<?= htmlspecialchars($article['thumbnail']) ?>"
                alt="<?= htmlspecialchars($article['alt_text']) ?>"
...
<?php
    foreach ($sections as $index => $section):
        ...
        // Assigning border colors to category sections
...
<section>
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold text-gray-900 border-l-4 border-<?= $borderColor ?> pl-2">
            <?= htmlspecialchars($category['name']) ?></h3>
        <a href="<?= $baseUrl ?>/categories/<?= htmlspecialchars($category['slug']) ?>"
            class="text-xs font-semibold text-blue-600 hover:text-blue-700">See All</a>
    </div>
